<script lang="ts">
  ...
  function handleTilt(e: MouseEvent) {
    ...
    card.style.transform = `perspective(1000px) rotateX(${y}deg) rotateY(${x}deg) scale(1.015)`;
...
<div 
  class="group glass-panel relative overflow-hidden {compact ? 'p-5' : 'min-h-[360px] grid lg:grid-cols-[1.05fr_0.95fr] gap-0 p-0'}"
...
>
  ...
  <div class={compact ? "" : "flex flex-col p-5 sm:p-7 relative z-10"}>
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <h3 class="mt-2 text-white section-title {compact ? 'text-xl' : 'text-2xl sm:text-3xl'}">
          {project.name}
        </h3>
      </div>
      {#if project.status}
    ...
    <p class="mt-4 leading-6 text-white/70 {compact ? 'text-sm' : 'text-base'}">
      {project.description[locale]}
    </p>
    
    {#if project.impact}
      <p class="mt-4 border-l-2 pl-4 text-sm leading-6 text-white/80" style="border-color: var(--c-fire)">
        {project.impact[locale]}
      </p>
    {/if}
    
    {#if project.role}
      <p class="mt-4 text-xs font-mono font-semibold uppercase leading-5 tracking-[0.14em] text-white/50">
        {project.role[locale]}
      </p>
    {/if}
    ...
    {#if !compact}
      <div class="mt-auto flex flex-wrap gap-3 pt-6">
        {#if project.github}
          <a href={project.github} data-cuelume-press="pulse" data-cuelume-release="release" target="_blank" rel="noreferrer" class="inline-flex min-h-11 items-center gap-2 rounded-full px-5 py-2.5 text-sm transition hover:scale-105" style="
        ...
        {/if}
        {#if project.demo}
          <a href={project.demo} data-cuelume-press="pulse" data-cuelume-release="release" target="_blank" rel="noreferrer" class="inline-flex min-h-11 items-center gap-2 rounded-full px-5 py-2.5 text-sm transition hover:scale-105" style="
  ...
  {#if !compact}
    <div class="hidden border-l relative p-4 lg:block" style="border-color: rgba(0,191,255,0.1); background: rgba(0,0,0,0.3)">
      <div class="relative h-full overflow-hidden rounded-xl border p-3 flex flex-col" style="
        border-color: rgba(0,191,255,0.2);
        background: rgba(0,0,0,0.5);
        box-shadow: inset 0 0 40px rgba(0,191,255,0.05);
      ">
        <div class="flex items-center justify-between border-b pb-2.5 mb-3" style="border-color: rgba(0,191,255,0.15)">
        ...
        {#if project.image?.src}
          <img 
            ...
            class="w-full aspect-[16/10] rounded-lg border object-cover" 
            style="border-color: rgba(0,191,255,0.2)"
          />
        {:else}
          <div class="w-full aspect-[16/10] rounded-lg border flex items-center justify-center" style="
        ...
        <div class="mt-3 grid grid-cols-3 gap-2">
          {#each (project.stack ?? project.tags).slice(0, 3) as item}
            <div class="rounded-lg border px-2 py-2.5 flex flex-col items-center text-center justify-center" style="
              border-color: rgba(0,191,255,0.15);
              background: rgba(0,191,255,0.03);
            ">
